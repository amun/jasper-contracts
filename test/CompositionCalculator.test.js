...
const getNumberWithDecimal = (num) => (new BigNumber(num).div(new BigNumber('10').pow(new BigNumber('18'))).toFixed(18));
...
describe("CompositionCalculator", function () {
  ...
  beforeEach(async function () {
    ...
    await this.token.initialize("InverseToken", "IT", 18);
  ...
  describe("#getNAV", function () {
    it("does match expected value", async function () {
      ...
      expect(getNumberWithDecimal(expectedNAV)).to.be.equal(getNumberWithDecimal(nav));
    });
  });
  ...
  describe("#getLendingFeeInCrypto", function () {
    it("does match expected value", async function () {
      ...
      const cryptoForLendingFee = await this.contract.getLendingFeeInCrypto(lendingFee, balance, daysSinceLastRebalance);

      //  const expectedResult = new BigNumber(lendingFee).div(100).div(365).times(daysSinceLastRebalance).times(balance).toFixed();
      const expectedResult = new BigNumber(2.5).div(100).div(365).times(1).times(100).toFixed(16);

      //is only same to 16th decimal?? Is this because of floating point error?
      expect((expectedResult)).to.be.equal(new BigNumber(getNumberWithDecimal(cryptoForLendingFee)).toFixed(16));
  ...
  describe("#getNeededChangeInBalanceToRebalance", function () {
    it("does match expected positive change", async function () {
      ...
      const result = await this.contract.getNeededChangeInBalanceToRebalance(nav, balance, price);
  ...
  describe("#calculateDailyPCF", function () {
    it("does not change when price stays and fee is zero", async function () {
      ...
      const [endNav, endBalance, endCashPosition, feeInFiat, changeInBalance, isChangeInBalanceNeg, cashFromCryptoSale] = Object.values(result);
  ...
  describe("#calculatePCF", function () {
    it("does not change changeInBalance when smaller then minRebalanceAmount", async function () {
      ...
      const result = await this.contract.calculatePCF(cashPosition, balance, price, lendingFee, daysSinceLastRebalance, minRebalanceAmount);
      const [endNav, endBalance, endCashPosition, feeInFiat, changeInBalance, isChangeInBalanceNeg, cashFromCryptoSale] = Object.values(result);
    ...
    it("does change positive when smaller price", async function () {
      ...
      const result = await this.contract.calculatePCF(cashPosition, balance, price, lendingFee, daysSinceLastRebalance, minRebalanceAmount);
      const [endNav, endBalance, endCashPosition, feeInFiat, changeInBalance, isChangeInBalanceNeg] = Object.values(result);

      expect(getNumberWithDecimal(changeInBalance)).to.be.equal('22.215372907153729022');
      expect(getNumberWithDecimal(feeInFiat)).to.be.equal('6.164383561643880000');
    ...
    it("does change positive when increased price", async function () {
      ...
      expect(getNumberWithDecimal(changeInBalance)).to.be.equal('18.188667496886675018');
      expect(getNumberWithDecimal(feeInFiat)).to.be.equal('7.534246575342520000');
      expect(new BigNumber(getNumberWithDecimal(endNav)).lt(100000)).to.be.true;
  ...
  describe("#getCurrentNAV", function () {
    it("Should match nav.", async function () {
      ...
      await this.storage.setAccounting(price, cashPosition, balance, fee, {from:owner});
  ...
  describe("#getCurrentTokenAmountCreatedByCash", function () {
    ...
    it("does give correct token amount for unbalanced product with lower spot price.", async function () {
      ...
      const cash = getEth(1200);
      const spot = getEth(800);
      const tokenCreated = await this.contract.getCurrentTokenAmountCreatedByCash(cash, spot);
      expect(getNumberWithDecimal(tokenCreated)).to.be.equal(getNumberWithDecimal(totalTokenSupply));
    });
  ...
  describe("#getCurrentCashAmountCreatedByToken", function () {
    ...
    it("does give correct cash payed for lower spot price.", async function () {
      ...
      const cashFromTokenRedeem = await this.contract.getCurrentCashAmountCreatedByToken(tokenAmount, spot);
